...
const DEFAULT_SPIN_SURE_DK = Math.max(5, Math.min(600, Number(process.env.KAFEPIN_SPIN_MINUTES || 45) || 45));
let SPIN_SURE_DK = DEFAULT_SPIN_SURE_DK;
...
const GUNLUK_SPIN_LIMIT = Math.max(1, Math.min(100, Number(process.env.KAFEPIN_SPIN_DAILY_LIMIT || 5) || 5));
...
(rows || []).forEach((r) => {
  ...
  db.get("SELECT value FROM settings WHERE key='sure'", (err, row) => {
    ...
    if (!row) {
      db.run("INSERT INTO settings (key,value) VALUES ('sure',?)", [String(DEFAULT_SPIN_SURE_DK)]);
      SPIN_SURE_DK = DEFAULT_SPIN_SURE_DK;
      return;
    }
...
function sendLiveMonitor() {
  ...
  db.all("SELECT * FROM sessions WHERE end_time=0", (e, rows) => {
    ...
    // Manual düzeltme / spin maliyeti de real_adjustments üzerinden anlık borca yansır.
            ...
            (paymentErr, paymentRow) => {
              ...
              getRangeStats(liveDayStart, liveDayEnd, (rangeErr, dailyStats) => {
                ...
                if (spinMaliyeti >= 0.005) {
                  txt += `\n🎁 Spin Maliyeti : -${spinMaliyeti.toFixed(2)} ₺ (PC/Masa ücretine dahil)`;
                }
...
const KAFEPIN_DESKTOP_APP_VERSION = "1.1.2";
...
app.post("/admin/pro/apply-local-update", (req, res) => {
  ...
  const picker = [
    ...
    "$owner.Close()",
    "$owner.Dispose()",
    "if($result -eq [System.Windows.Forms.DialogResult]::OK){[Console]::OutputEncoding=[Text.Encoding]::UTF8;Write-Output $d.FileName}"
  ].join(";");
...
function focusExplorerFolderWindows(folder, callback) {
  ...
  // Linux/test ortami icin sadece koruyucu fallback. KafePin saha kurulumu Windows'tur.
  if (process.platform !== "win32") {
    try {
  ...
  // Windows foreground kisiti yuzunden sadece explorer.exe <klasor> komutu gondermek
  // yeterli degildir. Hedef Explorer penceresinin HWND'si bulunur; pencere restore edilir,
  // foreground thread ile gecici input baglantisi kurulur ve HWND gercekten foreground
  // olana kadar kisa araliklarla yeniden denenir. Basari ancak GetForegroundWindow == HWND.
  const psScript = String.raw`
$ErrorActionPreference = 'SilentlyContinue'
$target = [System.IO.Path]::GetFullPath($env:KAFEPIN_FOCUS_FOLDER).TrimEnd('\\')
Add-Type @'
using System;
using System.Runtime.InteropServices;
public static class KafePinForeground {
  ...
  try {
    ...
    foreach ($window in @($shell.Windows())) {
      try {
        ...
        if ($candidate -ieq $target) { return $window }
      } catch {}
    }
  } catch {}
  return $null
...
function Force-Foreground([IntPtr]$hwnd) {
  try {
    [KafePinForeground]::ShowWindowAsync($hwnd, 9) | Out-Null # SW_RESTORE
    $foreground = [KafePinForeground]::GetForegroundWindow()
    [uint32]$pidA = 0
    [uint32]$pidB = 0
    $foregroundThread = if ($foreground -ne [IntPtr]::Zero) { [KafePinForeground]::GetWindowThreadProcessId($foreground, [ref]$pidA) } else { 0 }
    $targetThread = [KafePinForeground]::GetWindowThreadProcessId($hwnd, [ref]$pidB)
    $currentThread = [KafePinForeground]::GetCurrentThreadId()
    $attachedForeground = $false
    $attachedTarget = $false
    try {
      if ($foregroundThread -gt 0 -and $foregroundThread -ne $currentThread) {
        $attachedForeground = [KafePinForeground]::AttachThreadInput($currentThread, $foregroundThread, $true)
      }
      if ($targetThread -gt 0 -and $targetThread -ne $currentThread) {
        $attachedTarget = [KafePinForeground]::AttachThreadInput($currentThread, $targetThread, $true)
      }
      # Windows'un foreground-lock davranisini kullanici ALT girdisiyle serbest birak.
      [KafePinForeground]::keybd_event(0x12, 0, 0, [UIntPtr]::Zero)
      [KafePinForeground]::keybd_event(0x12, 0, 2, [UIntPtr]::Zero)
      [KafePinForeground]::SwitchToThisWindow($hwnd, $true)
      # Kisa TOPMOST -> NOTOPMOST dansi pencereyi Z-order'in basina tasir ama kalici topmost birakmaz.
      $flags = 0x0001 -bor 0x0002 -bor 0x0040
      [KafePinForeground]::SetWindowPos($hwnd, [IntPtr](-1), 0, 0, 0, 0, $flags) | Out-Null
      [KafePinForeground]::SetWindowPos($hwnd, [IntPtr](-2), 0, 0, 0, 0, $flags) | Out-Null
      [KafePinForeground]::BringWindowToTop($hwnd) | Out-Null
      [KafePinForeground]::SetActiveWindow($hwnd) | Out-Null
      [KafePinForeground]::SetForegroundWindow($hwnd) | Out-Null
      [KafePinForeground]::SetFocus($hwnd) | Out-Null
    } finally {
      if ($attachedTarget) { [KafePinForeground]::AttachThreadInput($currentThread, $targetThread, $false) | Out-Null }
      if ($attachedForeground) { [KafePinForeground]::AttachThreadInput($currentThread, $foregroundThread, $false) | Out-Null }
    }
  } catch {}
}
...
if (-not $window) {
  try { Start-Process -FilePath (Join-Path $env:WINDIR 'explorer.exe') -ArgumentList @($target) | Out-Null } catch {}
}
...
do {
  ...
  if ($window) {
    try {
      ...
      if ([KafePinForeground]::GetForegroundWindow() -eq $hwnd) {
        Write-Output ('FOCUSED|' + [string]$window.HWND)
        exit 0
      }
    } catch {}
  }
...
# Ilk acilis Explorer tarafinda gec kaldiysa bir kez daha shell'i tetikle ve zorla.
try { Start-Process -FilePath (Join-Path $env:WINDIR 'explorer.exe') -ArgumentList @($target) | Out-Null } catch {}
$retryDeadline = (Get-Date).AddSeconds(2.0)
do {
  $window = Find-TargetWindow
  if ($window) {
    try {
...
`;
  ...
  try {
    child = spawn("powershell.exe", [
      ...
      windowsHide: true,
      env: { ...process.env, KAFEPIN_FOCUS_FOLDER: folder },
  ...
  const timer = setTimeout(() => {
    try { child.kill(); } catch (_err) {}
    finishOnce(new Error("Explorer penceresi öne getirilemedi (odak zaman aşımı)"));
  }, 8000);
  ...
  child.on("close", code => {
    clearTimeout(timer);
    const match = stdout.match(/FOCUSED\|(\d+)/i);
    if (Number(code) === 0 && match) {
      return finishOnce(null, { verified: true, hwnd: Number(match[1]) || 0, method: "hwnd-foreground" });
    }
    finishOnce(new Error((stderr || stdout || "Explorer penceresi öne getirilemedi").trim()));
  });
}
    ...
    source.all(
      ...
      (sessionErr, sessionRows) => {
        ...
        const sessions = (sessionRows || []).filter((row) =>
          everyCafeTableNumber(row.ClientName) > 0 || isEveryCafeDirectSaleSession(row)
        );
  ...
  db.serialize(() => {
    db.run("BEGIN IMMEDIATE", (beginErr) => {
      ...
      db.run("DELETE FROM sessions WHERE masa=?", [masa], (sessionErr) => {
        if (sessionErr) return rollback(sessionErr);
        // EveryCafe "Beklemede" bilgisi yalnız oturum durumudur.
        // Index'in `masalar` çark sayacına/hakkına dokunulmaz.
        db.run("COMMIT", (commitErr) => {
          if (commitErr) return rollback(commitErr);
          delete aktifMasalar[masa];
          cb(null);
      ...
      const next = () => {
        ...
        const saveMapping = (mappingState = {}) => db.run(
          ...
          (mapErr) => {
            ...
            // KESİN SINIR (v3.0.22):
            // EveryCafe yalnız `sessions.start_time` (gerçek masa başlangıcı),
            // açık/kapalı/taşıma durumu ve ürün/gelir verisini sağlar.
            // `masalar.start_time` ÇARK zamanıdır ve EveryCafe/PING tarafından
            // ASLA başlatılmaz veya güncellenmez. İlk kayıt yalnız index'in
            // /api/status çağrısında, sonraki kayıt /api/spin sonrasında oluşur.
            ...
            db.run(
              ...
              (importErr) => {
                ...
                db.run("COMMIT", (commitErr) => {
                  ...
                  addEveryCafeIntegrationLog({category:"SALE",sessionId,event:"EveryCafe doğrudan satış",sourceDetail:`${total.toFixed(2)} ₺ • ${method} • ${orders.length} ürün`,action:"KafePin doğrudan satış/ödeme kaydına aktardı",result:"Başarılı • çift kayıt koruması aktif"});
                  ...
                  db.run("DELETE FROM everycafe_active_sessions WHERE session_id=?", [sessionId], (activeErr) => {
                    ...
                    db.run("INSERT INTO everycafe_imports(session_id,masa,source_end,total,imported_at) VALUES(?,?,?,?,?) ON CONFLICT(session_id) DO UPDATE SET masa=excluded.masa,source_end=excluded.source_end,total=0,imported_at=excluded.imported_at", [sessionId, masa, end, 0, Date.now()], (importErr) => {
                      ...
                      db.run("COMMIT", (commitErr) => {
                        ...
                        addEveryCafeIntegrationLog({category:"CLOSE",masa,sessionId,event:"Ücretsiz masa kapandı",sourceDetail:`EveryCafe kapanış: ${everyCafeIntegrationTimeText(end)} • 0 ₺`,action:"KafePin ücretsiz masa durumunu kapattı",result:"Başarılı • gelir yazılmadı"});
            ...
            db.run(
              ...
              (importErr) => {
                ...
                db.run("COMMIT", (commitErr) => {
                  ...
                  everyCafeOpenOrderFingerprints.delete(sessionId);
                  addEveryCafeIntegrationLog({category:"CLOSE",masa,sessionId,event:"Masa kapandı / tahsilat doğrulandı",sourceDetail:`EveryCafe kapanış: ${everyCafeIntegrationTimeText(end)} • ${total.toFixed(2)} ₺ • ${method}`,action:`KafePin finalize • bilgisayar ${computerTotal.toFixed(2)} ₺ + ürün ${productTotal.toFixed(2)} ₺`,result:"Başarılı • EveryCafe gerçek tahsilatı uygulandı",details:{startTime:localStart,endTime:end,total,computerTotal,productTotal,method}});
...
function everyCafeHistorySourceHash(sessionRows, orderRows, memberRows, otherPaymentRows) {
  const data = {
...
async function readEveryCafeHistorySource() {
  ...
  try {
    ...
    return {
      sessionRows, orderRows, memberRows, otherPaymentRows, deletionAuditRows,
      signature: everyCafeHistorySourceHash(sessionRows, orderRows, memberRows, otherPaymentRows)
    };
  } finally {
...
async function analyzeEveryCafeHistory(rawOptions = {}) {
  ...
  for (const row of source.otherPaymentRows||[]) {
    ...
    const sessionId=String(row.SessionID||"").trim();
    if (!paymentId || (sessionId && knownSessionIds.has(sessionId))) continue;
  ...
  return {
    ...
    sourceOtherPaymentRows:(source.otherPaymentRows||[]).length,sourceRecords:finalSourceItems.length,sourceTotal:finalSourceTotal,
            ...
            (todayErr, today) => {
              ...
              res.json({
                ...
                recent: sessions.map((row) => ({
                  ...
                  total: isEveryCafeFreeSession(row) ? 0 : (Number(row.PaymentAmount) || 0),
...
app.post("/admin/everycafe/start-test", (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run("INSERT INTO settings(key,value) VALUES('everycafe_sync_start_at',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", [String(startAt)], (err) => {
      ...
      addEveryCafeIntegrationLog({category:"SYSTEM",event:"EveryCafe canlı entegrasyon açıldı",sourceDetail:`Başlangıç: ${everyCafeIntegrationTimeText(startAt)}`,action:"KafePin salt-okunur canlı okumaya başladı",result:"Aktif"});
    ...
    (err, rows) => {
      ...
      list.forEach((sale) => {
        ...
        if (sale.sale_type === "DIRECT") {
          if (sale.external_source === "EVERYCAFE_DIRECT" || sale.external_source === "EVERYCAFE_OTHER") everyCafeDirectTotal += amount;
        ...
        (openErr, openRows) => {
          ...
          db.get(
            `SELECT COALESCE(SUM(total),0) AS total,
             ...
             FROM product_sales
             WHERE voided=0`,
            [monthStart, yesterdayStart, start, monthStart],
...
app.get("/admin/product-sales/list", async (req, res) => {
  ...
  const knownSessionIds = new Set((historySource.sessionRows||[]).map((r)=>String(r.SessionID||"")).filter(Boolean));
  const sourceSessionTotalFor = (rangeStart, rangeEnd) => Math.round((historySource.sessionRows || [])
    .filter((row) => {
      const sourceEnd = Number(row.EndDate) * 1000;
      return sourceEnd >= rangeStart && sourceEnd < rangeEnd && !isEveryCafeFreeSession(row);
    })
    .reduce((sum,row)=>sum+(Number(row.PaymentAmount)||0),0)*100)/100;
  const sourceMemberTotalFor = (rangeStart, rangeEnd) => Math.round((historySource.memberRows || [])
    .filter((row) => {
      const sourceTime = Number(row.PaymentDate) * 1000;
      return sourceTime >= rangeStart && sourceTime < rangeEnd;
    })
    .reduce((sum,row)=>sum+(Number(row.PaymentAmount)||0),0)*100)/100;
  const sourceOtherTotalFor = (rangeStart, rangeEnd) => Math.round((historySource.otherPaymentRows || [])
    .filter((row) => {
      const sessionId=String(row.SessionID||"").trim();
      if (sessionId && knownSessionIds.has(sessionId)) return false;
      const sourceTime=(Number(row.AddDate)||Number(row.UpdDate)||0)*1000;
      return sourceTime >= rangeStart && sourceTime < rangeEnd;
    })
    .reduce((sum,row)=>sum+(Number(row.PaymentAmount)||0),0)*100)/100;
  const sourceTotalFor = (rangeStart, rangeEnd) => Math.round((sourceSessionTotalFor(rangeStart,rangeEnd)+sourceMemberTotalFor(rangeStart,rangeEnd)+sourceOtherTotalFor(rangeStart,rangeEnd))*100)/100;
  db.all(
