...
try {
  ...
  # v3.1.56 CANDIDATE1 - hizli/kumulatif Yazici PRO uygulamasi.
  # Server Manager on-kontrolunun 35 sn timeout sinirina girmemek icin burada servis/WebView2 beklenmez.
  $YaziciPayload = Join-Path $InstallRoot 'v3156-yazici-payload'
  $YaziciRoot = Join-Path $InstallRoot 'KafePinYaziciPRO'
  $YaziciWeb = Join-Path $YaziciRoot 'web'
  $YaziciLog = Join-Path $LogDir 'v3156-yazici-apply.log'
  function YaziciLog([string]$Text) { try { Add-Content -LiteralPath $YaziciLog -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' [v3.1.56-candidate1] ' + $Text) -Encoding UTF8 } catch {} }
  function YaziciFileSha([string]$P) { return (Get-FileHash -Algorithm SHA256 -LiteralPath $P).Hash.ToLowerInvariant() }
  if (-not (Test-Path -LiteralPath $YaziciPayload -PathType Container)) { throw 'v3.1.56 Yazici payload klasoru bulunamadi.' }
...
YaziciLog 'KafePin ana masaustu uygulamasi v1.1.5 hazir.'
  [ordered]@{ version='3.1.56'; build='candidate1'; appliedAt=(Get-Date).ToString('o'); yaziciRoot=$YaziciRoot; runtimeActivation='on-first-launch'; updaterWait='none'; mobileQr='same-lan-token-only' } | ConvertTo-Json -Depth 3 | Set-Content -LiteralPath (Join-Path $SystemRoot 'v3156-yazici-applied.json') -Encoding UTF8
