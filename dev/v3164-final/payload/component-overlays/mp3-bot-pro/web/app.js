...
setTimeout(()=>{
  ...
  const tab=$('#winampModeBtn'),box=$('#winampMode');box.tabIndex=-1;let winampFilter='',winampSearchTimer=null,winampSearchOrigin='';
  ...
  const winampFilterBox=document.createElement('input');winampFilterBox.id='winampTrackFilter';winampFilterBox.type='search';winampFilterBox.placeholder='🔎 Bu arşivde sanatçı veya şarkı ara…';winampFilterBox.autocomplete='off';winampFilterBox.style.margin='8px 0';$('#winampCurrentFolder')?.parentElement?.insertBefore(winampFilterBox,locationBar.nextSibling);winampFilterBox.addEventListener('input',()=>{winampFilter=searchKey(winampFilterBox.value);clearTimeout(winampSearchTimer);winampSearchTimer=setTimeout(()=>safeAction(winampFilterBox,async()=>{if(!winampFilter){const origin=winampSearchOrigin;winampSearchOrigin='';if(origin)await browseFolder(origin,false);else render(window.winampActiveIndex);return}if(!winampSearchOrigin)winampSearchOrigin=window.winampCurrentFolder||window.winampTabSavedLocation;const d=await post('/api/winamp/search',{query:winampFilter,root:winampSearchOrigin});window.winampSource='winamp';window.winampFolderTracks=d.tracks||[];window.winampTracks=window.winampFolderTracks;window.winampFolders=[];window.winampActiveIndex=-1;render(-1);$('#winampCurrentFolder').textContent=`Hızlı arama: ${d.search_total||0} sonuç`;toast(`🔎 ${d.search_total||0} şarkı bulundu`)}),180)});
  ...
  async function browseFolder(path,remember=true,previousFolder='',asNewRoot=false){if(!path)throw new Error('Müzik klasörü bulunamadı.');if(previousFolder)rememberFolderSelection(previousFolder,path);const d=await post('/api/winamp/browse-folder',{path});if(asNewRoot)resetFolderHistory(d.folder||path);else if(remember)rememberFolder(d.folder||path);window.winampFocusedList='tracks';await applyFolderState(d,true)}
...
setTimeout(()=>{
  function setupMediaUsb(o){
    ...
    let sources=[],folder='',parent='',transaction=null,saved=[],savedSelected='',folderSelected='',fileSelected='',browserFocus='folder',selectedFilePaths=new Set();
...
setTimeout(()=>{
  ...
  let usbSources=[],usbBrowserFolder='',usbBrowserParent='',usbSavedLocations=[],usbSavedSelected='',usbFolderSelected='',usbFileSelected='',usbBrowserFocus='folder',usbSelectedFilePaths=new Set(),usbFilter='',lastUsbBrowser=null;
  const usbFilterBox=document.createElement('input');usbFilterBox.id='usbTrackFilter';usbFilterBox.type='search';usbFilterBox.placeholder='🔎 Sanatçı veya şarkı ara…';usbFilterBox.autocomplete='off';usbFilterBox.style.margin='8px 0';$('#usbBrowserRoots')?.insertAdjacentElement('afterend',usbFilterBox);usbFilterBox.addEventListener('input',()=>{usbFilter=searchKey(usbFilterBox.value);if(lastUsbBrowser)renderUsbBrowser(lastUsbBrowser)});
