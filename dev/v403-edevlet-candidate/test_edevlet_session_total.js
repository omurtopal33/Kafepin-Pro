...
(async()=>{
  ...
  if(tx.meta.active_session!==true) throw new Error("e-Devlet session is not active");
  const reused=prepareService("edevlet",1,"CASH",false,"e-Devlet Oturumu",10);
  if(reused.id!==tx.id) throw new Error("active session was duplicated");
  let s=state();
  s.initialized=true; s.lastSeenRecordId=100; s.jobs={};
  saveState(s);
  readEvents=()=>[{record_id:101,pages:1,printer:"TEST",document:"Adli Sicil"}];
  pollQueue(); tx=transaction(tx.id);
  if(tx.total!==20) throw new Error("first print total expected 20, got "+tx.total);
  readEvents=()=>[{record_id:101,pages:1,printer:"TEST",document:"Adli Sicil"},{record_id:102,pages:1,printer:"TEST",document:"İkametgâh"}];
  pollQueue(); tx=transaction(tx.id);
