(()=>{
  'use strict';
  if(window.__kafepinAI3154)return;
  window.__kafepinAI3154=true;
  ...
  function closeMobile(){const m=q('#ai3154Mobile');if(m)m.style.display='none';if(mobilePoll){clearInterval(mobilePoll);mobilePoll=null}}
  q('#ai3154MobileClose').onclick=closeMobile;
  q('#ai3153Camera').onclick=async()=>{
    try{
      ...
      q('#ai3154MobileQr').src=(d.qr_url||'')+'&_='+Date.now();
      q('#ai3154MobileUrl').textContent=d.url||'';
      q('#ai3154Mobile').style.display='flex';
