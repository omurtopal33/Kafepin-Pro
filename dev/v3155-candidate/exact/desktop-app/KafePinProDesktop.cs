...
namespace KafePinProDesktop
{
    ...
    internal sealed class MainForm : Form
    {
        ...
        private void UpdateNavButtons(string url)
        {
            ...
            SetActive(everyCafeIntegrationButton, !mp3ViewActive && !printerViewActive && !serviceViewActive && (value.EndsWith("/everycafe-integration.html") || value.Contains("/everycafe-integration.html?")));
            ...
            if (mp3ViewActive || printerViewActive || serviceViewActive)
            {
        ...
        private async void PrinterProButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string pythonw = Path.Combine(PrinterProRoot, ".venv", "Scripts", "pythonw.exe");
                    string service = Path.Combine(PrinterProRoot, "web_service.py");
                    if (!File.Exists(service))
                    {
                        MessageBox.Show(
                            "KafePin Yazıcı PRO kurulu değil.\n\nBeklenen klasör:\n" + PrinterProRoot +
                            "\n\nKümülatif güncelleme paketinin eksiksiz kurulduğunu kontrol edin.",
                            "KafePin Yazıcı PRO",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Warning);
                        return;
                    ...
                    {
                        printerProButton.Text = "🖨️ Yazıcı Kuruluyor...";
                        if (!await EnsurePrinterRuntimeAsync())
                        {
                            MessageBox.Show(
                                "Yazıcı PRO ilk kurulumu tamamlanamadı.\n\n" + PrinterProRoot + "\\KURULUM.cmd dosyasını çalıştırıp yeniden deneyin. Python 3 kurulumu gerekli olabilir.",
                                "KafePin Yazıcı PRO",
                                MessageBoxButtons.OK,
                                MessageBoxIcon.Error);
                            return;
                    ...
                    if (!await WaitForPrinterProAsync(20))
                    {
                        MessageBox.Show(
                            "Yazıcı PRO servisi 20 saniyede hazır olmadı.\n\n" + PrinterProRoot + " klasörünü ve Windows yazıcı/tarayıcı sürücülerini kontrol edin.",
                            "KafePin Yazıcı PRO",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                        return;
        ...
        {
            mp3ViewActive=false; printerViewActive=false; serviceViewActive=true; splash.Visible=false; browser.Visible=false; mp3Browser.Visible=false; printerBrowser.Visible=false; serviceBrowser.Visible=true; serviceBrowser.BringToFront(); UpdateNavButtons(ServiceProUrl);
        }
        ...
        private async Task EnsurePrinterBrowserAsync()
        {
            ...
            {
                int guard = 0;
                while (printerBrowserInitializing && guard++ < 100)
                    await Task.Delay(100);
                if (printerBrowserReady && printerBrowser.CoreWebView2 != null) return;
            }
            ...
            try
            {
                ...
                int wait = 0;
                while (!printerBrowserReady && wait++ < 150)
                    await Task.Delay(100);
                if (!printerBrowserReady)
                    throw new InvalidOperationException("Yazıcı PRO paneli WebView2 içinde yüklenemedi.");
            }
            finally
            {
                printerBrowserInitializing = false;
            }
        }
        ...
        {
            return mp3ViewActive || printerViewActive || serviceViewActive;
        }
