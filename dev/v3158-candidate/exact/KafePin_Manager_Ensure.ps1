...
try {
  ...
  # v3.1.57 CANDIDATE1 - hizli/kumulatif Yazici PRO uygulamasi.
  # Server Manager on-kontrolunun 35 sn timeout sinirina girmemek icin burada servis/WebView2 beklenmez.
  $YaziciPayload = Join-Path $InstallRoot 'v3157-yazici-payload'
  $YaziciRoot = Join-Path $InstallRoot 'KafePinYaziciPRO'
  $YaziciWeb = Join-Path $YaziciRoot 'web'
  $YaziciLog = Join-Path $LogDir 'v3157-yazici-apply.log'
  function YaziciLog([string]$Text) { try { Add-Content -LiteralPath $YaziciLog -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' [v3.1.57-candidate1] ' + $Text) -Encoding UTF8 } catch {} }
  function YaziciFileSha([string]$P) { return (Get-FileHash -Algorithm SHA256 -LiteralPath $P).Hash.ToLowerInvariant() }
  if (-not (Test-Path -LiteralPath $YaziciPayload -PathType Container)) { throw 'v3.1.57 Yazici payload klasoru bulunamadi.' }
  ...
  $pairs = @(
    [pscustomobject]@{ Src='index.html'; Dst='web\index.html'; Sha='bfa5fc9466271f5fc2c00bd85747ebbb1d85b92e369defa68dec3b9f554c278c' },
    ...
    [pscustomobject]@{ Src='KafePin_YaziciGelir_Service.js'; Dst='KafePin_YaziciGelir_Service.js'; Sha='ea26808efd7b4122503927b038209217808605a8b7c2dcfc9bab60f570df850a' },
    [pscustomobject]@{ Src='START_YAZICI_PRO.cmd'; Dst='START_YAZICI_PRO.cmd'; Sha='d8ca3ff183d7eb5f0d562a86697c26fb80590f7042e811d167c7e674e6fd3171' },
    [pscustomobject]@{ Src='KafePin_YaziciPRO_ServiceHost.ps1'; Dst='KafePin_YaziciPRO_ServiceHost.ps1'; Sha='bf81fcf5d9bbf3e3ce920cb60e35b79507597652a2aebacc49e7a882cf13c82e' },
    [pscustomobject]@{ Src='v3155-ai.js'; Dst='web\v3155-ai.js'; Sha='5cf40482e172af25b655d862e85ee87bf1a6ac811a7dbee5e4bf6ba8da815427' },
    ...
    [pscustomobject]@{ Src='web_service.py'; Dst='web_service.py'; Sha='b1e704329cd72046b82576cd919cec96780bc81ba96d10df7011c22d6521ad1e' },
    [pscustomobject]@{ Src='yazici-pro-version.json'; Dst='yazici-pro-version.json'; Sha='8db26084ca7fb091c5be9765d3e28d2a11638447e9abfdcfc73c95319b890ff0' }
  )
  ...
  foreach ($marker in @('v3155Verified','WHATSAPP WEB','BELGE / DİLEKÇE & AI','FOTOĞRAFTAN WORD','confirm3154Cancel','confirm3154Delete','ai3154Mobile','qjobdel','txdel')) {
    if ($indexRaw.IndexOf($marker,[StringComparison]::OrdinalIgnoreCase) -lt 0) { throw ('Yazici PRO v3.1.55 marker yok: ' + $marker) }
  }
...
YaziciLog 'KafePin ana masaustu uygulamasi v1.1.5 kuruluyor.'
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $desktopSetup -InstallRoot $InstallRoot -AppVersion '1.1.5'
if ($LASTEXITCODE -ne 0) { throw ('KafePin ana masaustu uygulamasi guncellenemedi. Cikis=' + $LASTEXITCODE) }
YaziciLog 'KafePin ana masaustu uygulamasi v1.1.5 hazir.'
  [ordered]@{ version='3.1.57'; build='candidate1'; appliedAt=(Get-Date).ToString('o'); yaziciRoot=$YaziciRoot; runtimeActivation='on-first-launch'; updaterWait='none'; mobileQr='same-lan-token-only' } | ConvertTo-Json -Depth 3 | Set-Content -LiteralPath (Join-Path $SystemRoot 'v3157-yazici-applied.json') -Encoding UTF8
  YaziciLog 'BASARILI: v3.1.55 dosyalar+SHA uygulandi; ayri Yazici PRO kisayolu kaldirildi; ana KafePin yenilemesi baslatildi.'
