...
try {
  ...
  # v3.1.55 CANDIDATE1 - hizli/kumulatif Yazici PRO uygulamasi.
  # Server Manager on-kontrolunun 35 sn timeout sinirina girmemek icin burada servis/WebView2 beklenmez.
  $YaziciPayload = Join-Path $InstallRoot 'v3155-yazici-payload'
  $YaziciRoot = Join-Path $InstallRoot 'KafePinYaziciPRO'
  $YaziciWeb = Join-Path $YaziciRoot 'web'
  $YaziciLog = Join-Path $LogDir 'v3155-yazici-apply.log'
  function YaziciLog([string]$Text) { try { Add-Content -LiteralPath $YaziciLog -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' [v3.1.55-candidate1] ' + $Text) -Encoding UTF8 } catch {} }
  function YaziciFileSha([string]$P) { return (Get-FileHash -Algorithm SHA256 -LiteralPath $P).Hash.ToLowerInvariant() }
  if (-not (Test-Path -LiteralPath $YaziciPayload -PathType Container)) { throw 'v3.1.55 Yazici payload klasoru bulunamadi.' }
  ...
  $pairs = @(
    ...
    [pscustomobject]@{ Src='KafePin_YaziciGelir_Service.js'; Dst='KafePin_YaziciGelir_Service.js'; Sha='b6726009b06b12ec86801ecd4c6755af41df4be14347d798b4ed5e4534bea7c3' },
    [pscustomobject]@{ Src='START_YAZICI_PRO.cmd'; Dst='START_YAZICI_PRO.cmd'; Sha='d56bb1e0da1f6fe61ada6e3c0d5e37e33102d8624cea3f1ed090530ca73b2fc6' },
    ...
    [pscustomobject]@{ Src='web_service.py'; Dst='web_service.py'; Sha='9b359cc2fa8402a505d5efe52acc8f8791a1ca29e2ec34ab9dba81f91cee7aee' },
...
YaziciLog 'KafePin ana masaustu uygulamasi v1.1.4 hazir.'
  [ordered]@{ version='3.1.54'; build='candidate1'; appliedAt=(Get-Date).ToString('o'); yaziciRoot=$YaziciRoot; runtimeActivation='on-first-launch'; updaterWait='none'; mobileQr='same-lan-token-only' } | ConvertTo-Json -Depth 3 | Set-Content -LiteralPath (Join-Path $SystemRoot 'v3155-yazici-applied.json') -Encoding UTF8
