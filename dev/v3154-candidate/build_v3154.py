...
def patch_candidate2(build: Path) -> None:
    ...
    start=payload/'START_YAZICI_PRO.cmd'; s=start.read_text(encoding='utf-8-sig')
    ...
    if old not in s: raise SystemExit('START external-browser fallback block not found')
    start.write_text(s.replace(old,new),encoding='utf-8')

    mgr=build/'KafePin_Manager_Ensure.ps1'; s=mgr.read_text(encoding='utf-8-sig').replace('3.1.54-candidate1','3.1.54-candidate2').replace("build='candidate1'","build='candidate2'")
    ...
    if old_sc not in s: raise SystemExit('desktop shortcut block not found')
    mgr.write_text(s.replace(old_sc,new_sc),encoding='utf-8-sig')

    html=payload/'index.html'; html.write_text(html.read_text(encoding='utf-8-sig').replace('__kafepinRevenue3153','__kafepinRevenue3154'),encoding='utf-8')
    vp=payload/'yazici-pro-version.json'; v=json.loads(vp.read_text(encoding='utf-8-sig')); v['version']='3.1.54'; v['build']='candidate2'
    ...
    v['fixes']=fixes; vp.write_text(json.dumps(v,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')

    up=build/'update.json'; u=json.loads(up.read_text(encoding='utf-8-sig'))
    u['notes']=("v3.1.54 ADAY / candidate2 — v3.1.49 STABLE tabanlı kümülatif güncelleme. İptal onay penceresini kapatır ve satış oluşturmaz; Sil Son Yazdırmalar/onay bekleyen kaydı gerçekten kaldırır; Ücretsiz korunur. Hem 'KafePin Yazıcı PRO' hem eski 'KafePin Yazici PRO' masaüstü kısayolu aynı WebView2 launcher'a zorla bağlanır. WhatsApp Web yalnız Yazıcı PRO iç sekmesinde çalışır; dış tarayıcı fallback'i yoktur. Fotoğraf Çek aynı ağda 5 dakikalık tek kullanımlık QR ile telefondan doğrudan ana makineye gönderir; diğer API'ler LAN'a kapalıdır. EveryCafe salt-okunur ve STABLE v3.1.49 kalır.")
    ...
    for f in sorted(payload.rglob('*')):
        ...
        if n!=1: raise SystemExit('manager SHA pair not found: '+src)
    mgr.write_text(s,encoding='utf-8-sig')

def main():
    if not BASE.exists(): raise SystemExit(f'base missing: {BASE}')
    encoded=''.join(p.read_text(encoding='ascii').strip() for p in sorted(DEV.glob('delta.b64.part*')))
    ...
    with tempfile.TemporaryDirectory(prefix='kp3154-') as td:
        ...
        (build/'server.js').write_bytes(server); patch_candidate2(build)
    ...
    latest={'version':'3.1.54','channel':'candidate','stableVersion':'3.1.49','baseVersion':'3.1.49','cumulative':True,'publishedAt':'2026-08-21T10:35:00+03:00','notes':update['notes'],'downloadUrl':'https://raw.githubusercontent.com/omurtopal33/Kafepin-Pro/main/KafePin-Pro-Update-v3.1.54.zip','sha256':digest}
    LATEST.write_text(json.dumps(latest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); shutil.copyfile(REPORT_SRC,REPORT_OUT)
